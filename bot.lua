...
http.TIMEOUT = 10
-------------------------------------------------------
Mehdi = 487448446     --در این قسمت ایدی عددی ربات را وارد کنید
-------------------------------------------------------
bot_id = 487448446     --در این قسمت ایدی عددی ربات را وارد کنید
-------------------------------------------------------
sudo_users = {309573480}    -- در ای قسمت ایدی مدیر اصلی
-------------------------------------------------------
...
-------------------------------------------------------
if txt:match("^حفظ$") and reply_id and is_sudo(msg) then
...
if txt:match("^رفع مطور$") and reply_id and is_ultrasudo(msg) then
...
-------------------------------------------------------
if txt:match("^تنزيل مطور$") and reply_id and is_ultrasudo(msg) then
...
-------------------------------------------------------
if txt:match("^اضافه للكل$") and msg.reply_to_message_id_ ~= 0 and is_sudo(msg) then
...
-------------------------------------------------------
if txt:match("^المجموعات$") and is_sudo(msg) then
meti = redis:scard(Mehdi.."sgps:")
tdcli.sendText(chat_id, msg.id_, 0, 1, nil, 'میدونستی امار من اینقده؟ '..meti..' 😻😹😹😹', 1, 'html')
end
-------------------------------------------------------
if txt:match("^غادر من الكل$") and is_sudo(msg) then
...
tdcli.sendText(chat_id, msg.id_, 0, 1, nil, 'لفت دادم داوش', 1, 'html')
...
-------------------------------------------------------
if txt:match("^مشاركه$") and is_sudo(msg) then
tdcli.sendChatAction(msg.chat_id_,'Typing')
...
-------------------------------------------------------
if txt:match("^الدخول تلقائي تفعيل$") and is_sudo(msg) then
...
-------------------------------------------------------
if txt:match("^الدخول تلقائي تعطيل$") and is_sudo(msg) then
if redis:get(Mehdi.."autojoin") then
